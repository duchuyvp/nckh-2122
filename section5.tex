...
\subsubsection{Thuật toán Nhân đôi và cộng}

\textcolor{red}{Thuật toán bình phương và nhân tính $a^n \mod m$}
...
Có vẻ khó để tìm ra giá trị $n$ từ 2 điểm $P, Q$ và phương trình $Q=nP$, hay giải một bài toán elliptic logarit rời rạc. Ta sẽ thảo luận về độ phức tạp của ECDLP ở phần sau.
Để sử dụng hàm
$$ \mathbb{Z} \longrightarrow E(\mathbb{F}_p), \quad \quad n \longmapsto nP,$$
trong mật mã, ta cần tính $nP$ từ hai giá trị đã biết trước là $n$ và $P$. Nhưng nếu $n$ lớn, hoặc rất lớn, chắc chắn ta không muốn tính $nP$ bằng cách tính $P, 2P, 3P, 4P, \ldots $

Cách hiệu quả nhất để tính $nP$ cũng tượng tự cách tính $a^n \pmod{m$}. Tuy nhiên, các phép toán trên đường cong elliptic được viết như phép cộng nên chúng tôi gọi
	thuật toán là nhân đôi và cộng thay vì bình phương và nhân.
	Với ý tưởng tương tự, chúng tôi mô tả thuật toán như sau


	\begin{tabular}{|rl|}
		% \centering
		\hline
		            & \textbf{Đầu vào:} Điểm $P \in E(\mathbb{F}_p)$ và số nguyên $n \geq 1$. \\
		\textbf{1.} & Đặt $Q=P$ và $R=\mathcal{O}$                                            \\
		\textbf{2.} & Lặp lại khi $n > 0$.                                                    \\
		            & \textbf{3.} Nếu $n \equiv 1 \pmod{2}$, đặt $R = R+Q$.                   \\
		            & \textbf{4.} Đặt $Q = 2Q$ và $n = \lfloor \frac{n}{2} \rfloor$.          \\
		            & \textbf{5.} Nếu $n > 0$ tiếp tục vòng lặp và trở lại \textbf{bước 2}    \\
		\textbf{6.} & Trả về điểm $R$, là kết quả của $nP$.                                   \\
		\hline
	\end{tabular}
	...
	ứng dụng đơn giản nhất, trao đổi khóa Diffie - Hellman, %  liên quan đến nhiều hơn
